            ...
            <b-row v-if="selected.Status == 'pending'">
              <b-col sm="12">
                ...
                <select class="form-control" v-model="Vehicle_Name">
                  ...
                  <option v-for="make in list" :key="make.ID" v-bind:value="make.ID">
                    {{ make.name }}
                  </option>
                </select>
              </b-col>
...
export default {
  ...
  computed: {
    list() {
      consola.success(this.$store.state.VendorliveVehicleslist)
      return this.$store.state.VendorliveVehicleslist
    },
    rowss() {
  ...
  created() {
    this.$store.dispatch('get_VendorliveVehicleslist')
  },
  beforeCreate() {
