...
const createStore = () => {
  return new Vuex.Store({
    state: {
      ...
      VendorDriversList: [], // it is for make vehicle live popup------27--01-2019
      VendorliveVehicleslist: [],
      MyLiveVehicles: {},
      adda_list: []
    },
    getters: {
    ...
    mutations: {
      ...
      setVendorliveVehicleslist: (state, payload) => (state.VendorliveVehicleslist = payload), // For Showing in make vehicle live popup
        ...
        DB.ref('users')
          ...
          .then((snap) => {
            ...
            snap.forEach((ele) => {
              ...
              if (ele.val().vehicle === 'Others') {
                console.log('tttt')
                payload.push({
                  ID: ele.key,
                  first_name: ele.val().first_name,
                  last_name: ele.val().last_name
                })
              }
            })
            commit('setVendorDriversList', payload)
          })

        // ((snap) => commit('setVendorDriversList', snap.val()))
      },
      get_VendorliveVehicleslist({ commit, state }) {
        DB.ref('heavyVehicles')
          ...
          .then((snap) => {
            ...
            commit('setVendorliveVehicleslist', payload)
          })
